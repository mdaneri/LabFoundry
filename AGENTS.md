...
- Real DNS/DHCP apply stages rendered dnsmasq config under `/var/lib/labfoundry/apply/dnsmasq/labfoundry.conf` as the `labfoundry` service user before invoking the root helper. The helper validates with `dnsmasq --test`, installs `/etc/labfoundry/dnsmasq.d/labfoundry.conf`, manages the LabFoundry dnsmasq systemd drop-in, enables `dnsmasq`, reloads/restarts the service, and points the appliance's own systemd-resolved path at local dnsmasq. Validate actual DNS with direct queries such as `Resolve-DnsName <name> -Server <appliance-ip>` or `dig @<appliance-ip> <name>`, plus in-guest `getent hosts <name>` for appliance-local resolution, not only the UI preview.
