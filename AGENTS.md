...
- Real Appliance Settings apply stages JSON under `/var/lib/labfoundry/apply/appliance-settings/labfoundry-settings.json` as the `labfoundry` service user before invoking the root helper. The helper validates resolver mode, management interface/IP, NTP servers, and any management UI HTTPS cert/key paths; sets the OS hostname to the appliance FQDN; local DNS mode sets management resolver DNS to `127.0.0.1` and `Domains=~.`; external DNS mode uses configured resolver servers and removes the catch-all domain; NTP is rendered through Photon `systemd-timesyncd`; and CA-backed management HTTPS writes `/etc/systemd/system/labfoundry.service.d/management-https.conf`, starts `labfoundry-http-redirect.service` on port 80, reloads systemd, then schedules a short delayed `labfoundry.service` restart so the apply job can be recorded.
