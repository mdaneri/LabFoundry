...
- Appliance Settings owns appliance FQDN, OS hostname, appliance resolver mode/servers, management UI HTTPS preference, and the appliance NTP client. DNS/DHCP owns rendered DNS records and dnsmasq reload, not appliance resolver or hostname enforcement.
...
- Photon image provisioning creates the bootstrap admin OS account under `/var/lib/labfoundry/users` with `/sbin/nologin` and the bootstrap admin password, so the default admin has a real Photon account before first apply.
