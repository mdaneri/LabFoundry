...
ESXi PXE stores Kickstart source files in the LabFoundry database. The database is the source of truth; generated files under `/var/lib/labfoundry/pxe/http/esxi/ks/<id>.cfg` are runtime copies for PXE HTTP boot only. Saving in the CodeMirror editor updates desired state and marks the `esxi_pxe` apply unit changed. Global appliance apply stages `/var/lib/labfoundry/apply/esxi-pxe/labfoundry-esxi-pxe.json`, writes enabled Kickstarts to the PXE HTTP root, removes stale generated numeric `.cfg` files, records render/apply timestamps, and redacts sensitive Kickstart values from previews, diffs, jobs, logs, and audit events.
