...
- lists changed apply units such as Local Users, Appliance Settings, Network, DNS/DHCP, Firewall, KMS, VCF Backups, VCF Offline Depot, and VCF Private Registry;
...
- Local Users apply stages `/var/lib/labfoundry/apply/local-users/labfoundry-users.json`, creates enabled local users as non-interactive Photon OS accounts under `/var/lib/labfoundry/users`, locks disabled users, and clears pending encrypted OS passwords only after a successful real apply.
