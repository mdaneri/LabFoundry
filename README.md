...
ESXi PXE stores Kickstart source files in the LabFoundry database. The database is the source of truth; generated files under `/var/lib/labfoundry/pxe/http/esxi/ks/<id>.cfg` are runtime copies for PXE HTTP boot only. Saving in the CodeMirror editor updates desired state and marks the `esxi_pxe` apply unit changed. Installer ISO choices are discovered from `/mnt/labfoundry-vcf-offline-depot/PROD/COMP/ESX_HOST`, the VCFDT ESX host component folder; LabFoundry creates that folder when needed and lets operators upload additional `.iso` files there. Host PXE definitions can reference both a database Kickstart and a selected installer ISO path. Global appliance apply stages `/var/lib/labfoundry/apply/esxi-pxe/labfoundry-esxi-pxe.json`, writes enabled Kickstarts to the PXE HTTP root, removes stale generated numeric `.cfg` files, validates selected ISO paths stay under the ESX_HOST folder, records render/apply timestamps, and redacts sensitive Kickstart values from previews, diffs, jobs, logs, and audit events.
