...
Audit Events is a separate Operations page because it is structured history rather than stream output. Its read-only Tabulator grid fills the available viewport and uses local pagination sized automatically from the visible grid height, without a fixed row count or page-size selector. Column filters and wrapped detail cells remain inside the grid without crowding the Logs tabs or scrolling the parent page.
