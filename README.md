...
ESXi PXE stores Kickstart source files in the LabFoundry database. The database is the source of truth; generated files under `/var/lib/labfoundry/pxe/http/esxi/ks/<id>.cfg` are runtime copies for PXE boot only. Saving in the CodeMirror editor updates desired state and marks the `esxi_pxe` apply unit changed. ESXi PXE boot settings select a DHCP IP zone instead of a freeform interface/IP pair; LabFoundry derives the PXE interface, TFTP server address, DNS record, firewall bind target, and dnsmasq scope tag from that zone. Installer ISO choices are discovered from `/mnt/labfoundry-vcf-offline-depot/PROD/COMP/ESX_HOST`, the VCFDT ESX host component folder; LabFoundry creates that folder when needed and lets operators upload additional `.iso` files there. Host PXE definitions can reference both a database Kickstart and a selected installer ISO path. Global appliance apply stages schema-v2 `/var/lib/labfoundry/apply/esxi-pxe/labfoundry-esxi-pxe.json`, validates selected ISO paths stay under the ESX_HOST folder, extracts selected installers to `/var/lib/labfoundry/pxe/http/esxi/images/<image-key>/`, generates host-specific `boot.cfg` and PXELINUX configs, stages `undionly.kpxe`, `snponly.efi`, `pxelinux.0`, `mboot.efi`, and `mboot.c32`, installs a dedicated static ESXi PXE HTTP listener on port `8080` by default, records render/apply timestamps, and redacts sensitive Kickstart values from previews, diffs, jobs, logs, and audit events. The helper searches Photon package paths plus `/var/lib/labfoundry/pxe/bootloaders` for the iPXE/SYSLINUX first-stage files; stage `undionly.kpxe` and `snponly.efi` there if the appliance package stream does not ship them. When ESXi PXE boot settings change, review and apply the changed DNS/DHCP, ESXi PXE, and Firewall units together so dnsmasq, generated boot artifacts, and UDP/69 plus TCP/8080 allow rules stay aligned.
