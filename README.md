...
Audit Events is a separate Operations page because it is structured history rather than stream output. Its read-only Tabulator grid fills the available viewport, uses virtual vertical rendering with an internal scrollbar, and provides column filters and wrapped detail cells without crowding the Logs tabs or scrolling the parent page.
