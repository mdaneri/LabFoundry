...
DHCP IP zones can bind only to valid IPv4 service targets: access physical interfaces with an IPv4 CIDR or enabled VLAN interfaces with an IPv4 CIDR. Trunk physical interfaces, IPv6-only interfaces, and addressless interfaces are rejected before apply. The rendered dnsmasq config owns DHCP ranges, options, reservations, and the lease file at `/var/lib/labfoundry/dnsmasq/dhcp.leases`; live lease readback goes through the allowlisted `labfoundry-helper dnsmasq leases --real` path.
...
The ESXi PXE boot service selects one or more DHCP IP zones instead of a freeform interface/IP pair. LabFoundry derives the PXE interfaces, TFTP server addresses, DNS records, firewall bind targets, and generated dnsmasq scope tags from those zones; the DHCP page shows those generated PXE lines separately from operator-managed DHCP options. Native UEFI HTTP boot URLs are generated per selected zone from that zone's appliance address unless an operator supplies a manual absolute URL, in which case the same URL is returned in every selected zone.
