...
Service-owned endpoint DNS uses a canonical alias plus per-interface records. When a service such as KMS, ESXi PXE, VCF Offline Depot, or VCF Private Registry selects one or more listen interfaces, LabFoundry creates direct A/AAAA records for interface-specific names such as `depot-eth2.labfoundry.internal` and makes the main service name, such as `depot.labfoundry.internal`, a CNAME to the first selected interface name. This alias is created even for a single selected interface. Operators can use the canonical name by default and use the interface-specific names for networks that cannot reach the first selected interface. LabFoundry does not create a multi-A default for the canonical service name because clients may resolve an unreachable address. If the first selected interface is removed or the interface order changes, the CNAME target moves to the next first valid selected interface and stale app-owned interface records are removed.
...
The real KMS apply path is PyKMIP-backed and lab-only. The KMS page derives IPv4 and IPv6 listen addresses from the selected access physical interface or enabled VLAN, creates app-owned canonical CNAME and per-interface A/AAAA records for the KMIP hostname, and requires an enabled healthy CA before KMS can be activated. When KMS is enabled, CA desired state auto-ensures the `kms:server` certificate and enabled KMIP client certificates; apply remains invalid until the issued server certificate and key are available.
